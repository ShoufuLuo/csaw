...
void key_init ()
{
    ...
    for (i = 0; i < 5; i++)
	{
	    gKey_Max.t[i] = ULONG_MAX;
	    gKey_Half.t[i] = ULONG_MAX;
	}
    gKey_Half.t[4] = gKey_Half.t[4] / 2;

    key_to_str (&gKey_Max);
    key_to_str (&gKey_Half);
		   ...
		   const Key * const k2)
{
    ...
    comp = key_comp (diff, &gKey_Half);

    /* diff > Key_Half */
    if (comp > 0)
	key_sub (logs, diff, &gKey_Max, diff);
...
void key_midpoint (void *logs, Key * mid, Key key)
{
    ...
    if (key_comp (&key, &gKey_Half) < 0)
	key_add (mid, &key, &gKey_Half);
    else
	key_sub (logs, mid, &key, &gKey_Half);
