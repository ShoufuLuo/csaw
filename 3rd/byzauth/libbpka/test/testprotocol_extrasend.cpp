...
std::multimap<unsigned, MessageTrace>    message_trace; 
...
int main(int argc, char * argv[]) 
{
   if( argc != 8 ) 
   {
      std::cout << "Usage " << argv[0] << " <bsfile> <msgtrace> <tgs> <expiryinterval> <nmesgayload> <keysz> <retryinterval>" << std::endl;  
      return 1; 
   }
   ...
   int retryinterval = atoi(argv[7]); 
   std::cout << "resend at interval " << retryinterval << std::endl ;
   ...
   std::cout << "initialize all adapters" << std::endl ;
        ...
        // add dummy messages to trace 
   std::multimap<unsigned, MessageTrace> extra_message_trace; 

   std::map<std::string, unsigned> last_sent_time; 
   for( std::set<std::string>::iterator j = peers.begin() ; j != peers.end() ; j++ ) 
      last_sent_time[*j] = 0; 

   unsigned nextintrospection = message_trace.begin()->second.date + retryinterval ; 
   for( std::multimap<unsigned, MessageTrace>::iterator i = message_trace.begin(); 
        i != message_trace.end() ; i++ ) 
   {
      for(  ; i != message_trace.end() && i->second.date < nextintrospection ; i++ )
         last_sent_time[ i->second.from ] =  i->second.date ;
      for( std::set<std::string>::iterator j = peers.begin() ; j != peers.end() ; j++ )   
         if( last_sent_time[*j] < (nextintrospection - retryinterval) )
         {
            last_sent_time[*j] = i->second.date ;
            MessageTrace m; 
            m.from = *j ;
            m.to = trustgroups[*j];
            m.date = i->second.date;
            m.size = 0; // special size 
            extra_message_trace.insert( std::make_pair( i->second.date, m ) ) ;  
         }
      nextintrospection += retryinterval; 
   }

   std::cout << "introspection done " << std::endl ;
   std::cout << "adding " << extra_message_trace.size() << " messages to " << 
      message_trace.size() << std::endl ;

   for( std::multimap<unsigned, MessageTrace>::iterator i = extra_message_trace.begin(); 
        i != extra_message_trace.end() ; i++ ) 
      message_trace.insert( std::make_pair(i->first, i->second) ) ; 

   extra_message_trace.clear();
        ...
        i != message_trace.end() ; i++ ) 
   {
      ...
      fprintf( statfp, "SENDER=%s  TIME=%lu  NUMDEST=%lu OVERHEAD=%d  BASEOVERHEAD=%lu SIZE=%lu  COMPLETION=%f NTRUSTED=%u NKNOWN=%u CACHENMESG=%u CACHENBYTES=%u\n" , 
               i->second.from.c_str(), 
